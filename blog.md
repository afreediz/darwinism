...
*produce*. A decision isn't a category like `EAT` or `FLEE`. It's a row of six numbers:
...
Notice what these all have in common: they're **continuous and differentiable.** A network
outputs a heading as two smooth numbers, not a choice from a menu. It outputs "how much do I
want to eat" as a probability, not a yes/no. This is the difference between an action space a
neural network can learn by gradient descent and one it can only fumble at. Hard branches
kill gradients; soft gates let them flow.

## The world pushes back: propose vs. dispose

Now, here's a subtlety that took me a while to get right. If the brain says "eat," should the
...
impossible happen. The brain is free to be a messy approximator, because the systems are the
...
breeds everyone — and finally writes down what happened. Always this order. The order itself
is part of the run's fingerprint.*

The environment updates. A spatial index is rebuilt. Perception is gathered. Brains decide.
The sleep system overrides anyone who should be resting. Then the consequences roll out:
movement, eating and hunting, metabolism (burning energy, starving, dying), reproduction,
